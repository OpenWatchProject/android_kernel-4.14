...
static void tpd_swap_left_right(int *x, int *y)
{
	int temp = 0;
	temp = TPD_RES_X - *x;
	*x = temp;
...
static void tpd_swap_up_down(int *x, int *y)
{
	int temp = 0;
	temp = TPD_RES_Y - *y;
	*y = temp;
...
static void tpd_swap_x_y(int *x, int *y)
{
	int temp = 0;
...
static int fts_input_report_b(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
		...
		#if defined(TPD_X_Y_SWAP)
			tpd_swap_x_y(&events[i].x, &events[i].y);
		#endif
		
		#if defined(TPD_UP_DOWN_SWAP)
			tpd_swap_up_down(&events[i].x, &events[i].y);
		#endif

		#if defined(TPD_LEFT_RIGHT_SWAP)
			tpd_swap_left_right(&events[i].x, &events[i].y);
		#endif
