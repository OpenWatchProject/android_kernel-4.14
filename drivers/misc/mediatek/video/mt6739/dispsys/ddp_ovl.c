...
static int ovl_layer_config(enum DISP_MODULE_ENUM module, unsigned int layer,
			    ...
			    enum LCM_PHYSICAL_ROTATION physical_rotation, void *handle)
{
	...
	if (physical_rotation != LCM_PHYSICAL_ROTATION_NONE)
		rotate = 1;
		...
		if (pConfig->ovl_partial_dirty) {
			...
			if (rect_intersect(&layer_roi,
				...
				ovl_layer_config(module, ovl_cfg->phy_layer,
						 ...
						 &pConfig->ovl_partial_roi,
						 &layer_partial_roi, pConfig->dispif_config.dsi.physical_rotation, handle);
			} else {
			...
			ovl_layer_config(module, ovl_cfg->phy_layer,
					 ...
					 pConfig->dispif_config.dsi.physical_rotation, handle);
		}
