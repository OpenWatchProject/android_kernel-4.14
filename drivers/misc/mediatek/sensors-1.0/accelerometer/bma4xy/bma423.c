...
uint16_t bma423_dump_reg(unsigned char *res_buf, struct bma4_dev *dev)
{
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA423_CHIP_ID) {
			rslt = bma4_read_regs(BMA4_FEATURE_CONFIG_ADDR, feature_config, BMA423_FEATURE_SIZE, dev);
			if (rslt == BMA4_OK) {
				rslt = snprintf(res_buf,PAGE_SIZE,"\n reg  0x5E: \n");
				for(temp_val = 0; temp_val < BMA423_FEATURE_SIZE ; temp_val++)
				{
					rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"0x%02x   ",feature_config[temp_val]);
					if(0 == (temp_val+1)%10)
						rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"\n");
				}
			}
		} else {
			rslt = 0;
	...
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"\n reg  0x5F: \n");
	read_rslt = bma4_read_regs(BMA4_INTERNAL_ERROR, feature_config, 1, dev);
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"0x%02x  \n",feature_config[0]);

	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"\n reg  0x53: \n");
	read_rslt = bma4_read_regs(BMA4_INT1_IO_CTRL_ADDR, feature_config, 1, dev);
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"0x%02x  ",feature_config[0]);

	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"\n reg	0x55: \n");
	read_rslt = bma4_read_regs(BMA4_INTR_LATCH_ADDR, feature_config, 1, dev);
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"0x%02x	",feature_config[0]);

	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"\n reg  0x56: \n");
	read_rslt = bma4_read_regs(BMA4_INT_MAP_1_ADDR, feature_config, 1, dev);
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"0x%02x  ",feature_config[0]);
	
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"\n reg  0x2A: \n");
	read_rslt = bma4_read_regs(BMA4_INTERNAL_STAT, feature_config, 1, dev);
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"0x%02x  ",feature_config[0]);

	
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"\n reg  0x02: \n");
	read_rslt = bma4_read_regs(BMA4_ERROR_ADDR, feature_config, 1, dev);
	rslt+= snprintf(res_buf+rslt,PAGE_SIZE,"0x%02x  ",feature_config[0]);
